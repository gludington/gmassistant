...
export function PlaylistDrawer({
  ...
  const updateTrack = useMutation({
    mutationFn: async ({ id, name, url, type }: { id: number; name: string; url: string; type: 'file' | 'youtube' | 'spotify' }) => {
            ...
            <div key={pl.id} style={{ ...s.card, ...(isPlaying ? s.cardPlaying : {}) }}>
              ...
              {renamingId === pl.id && (
                <form
                  ...
                  onSubmit={(e) => {
                    e.preventDefault();
                    if (renameName.trim()) renamePlaylist.mutate({ id: pl.id, name: renameName.trim() });
                  }}
                >
                  <input
...
function TrackList({ tracks, onDelete, onReorder, onUpdate }: {
  ...
  onUpdate: (id: number, data: { name: string; url: string; type: 'file' | 'youtube' | 'spotify' }) => void;
}) {
          ...
          {editingId === track.id ? (
            <div style={{ ...s.trackRow, flexDirection: 'column', alignItems: 'stretch', gap: 4, padding: '6px 8px', cursor: 'default' }}>
              ...
              {track.type !== 'file' && (
                <input
                  ...
                  placeholder={track.type === 'spotify' ? 'Spotify URL' : 'YouTube URL'}
                  onKeyDown={(e) => e.key === 'Escape' && setEditingId(null)}
                />
              )}
            ...
            >
              ...
              <span style={{ fontSize: '0.72rem', flexShrink: 0, color: '#666' }}>
                {track.type === 'youtube' ? '▶' : track.type === 'spotify' ? '♫' : '🎵'}
              </span>
...
function AddTrackForm({ playlistId, onSuccess, onCancel }: {
  ...
  const [type, setType] = useState<'file' | 'youtube' | 'spotify'>('youtube');
  ...
  const addTrack = useMutation({
    mutationFn: async (data: { name: string; type: 'file' | 'youtube' | 'spotify'; url: string }) => {
  ...
  return (
    <div style={s.addTrackPanel}>
      <select style={s.input} value={type} onChange={(e) => setType(e.target.value as 'file' | 'youtube' | 'spotify')}>
      ...
      {(type === 'youtube' || type === 'spotify') ? (
        <form onSubmit={(e) => { e.preventDefault(); if (name.trim() && url.trim()) addTrack.mutate({ name: name.trim(), type, url: url.trim() }); }}>
          ...
          <input style={{ ...s.input, marginTop: 6 }} placeholder={type === 'spotify' ? 'Spotify track/album/playlist URL' : 'YouTube URL'} value={url} onChange={(e) => setUrl(e.target.value)} />
          <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
